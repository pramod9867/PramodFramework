...
class BLPanCompleteDetails extends StatefulWidget {
  final bool isPanDetailsFilled;
  final String flag;

  const BLPanCompleteDetails(
      {Key? key, this.isPanDetailsFilled = true, this.flag = ""})
      : super(key: key);
  @override
  State<StatefulWidget> createState() => _BLPanCompleteDetailsState();
}

class _BLPanCompleteDetailsState extends State<BLPanCompleteDetails>
  ...
  GlobalKey<_BLPanCompleteDetailsState> _scrollViewKey = GlobalKey();
    ...
    if (true) {
      firstEditingController.text =
          panDetailsBloc?.panResponseDTO?.firstName != null
              ...
              : blFetchBloc!.fetchBLResponseDTO.panFirstName != null
              ? blFetchBloc!.fetchBLResponseDTO.panFirstName!
              : "";
      middleEditingController.text =
          panDetailsBloc?.panResponseDTO?.middleName != null
              ? panDetailsBloc!.panResponseDTO!.middleName!
              : blFetchBloc!.fetchBLResponseDTO.panMiddleName != null
                  ? blFetchBloc!.fetchBLResponseDTO.panMiddleName!
                  : "";
      lastEditingController.text =
          panDetailsBloc?.panResponseDTO?.lastName != null
              ? panDetailsBloc!.panResponseDTO!.lastName!
              : blFetchBloc!.fetchBLResponseDTO.panLastName != null
                  ? blFetchBloc!.fetchBLResponseDTO.panLastName!
                  : "";
      ;
      dateController.text = panDetailsBloc?.panResponseDTO?.dateOfBirth != null
          ? panDetailsBloc.panResponseDTO!.dateOfBirth!
          : blFetchBloc!.fetchBLResponseDTO.dateOfBirthPan != null
              ? blFetchBloc!.fetchBLResponseDTO.dateOfBirthPan!
              : "";
      aadhaarEditingController.text =
          panDetailsBloc?.panResponseDTO?.panNumber != null
              ? panDetailsBloc.panResponseDTO!.panNumber!
              : blFetchBloc!.fetchBLResponseDTO.panNumber != null
                  ? blFetchBloc!.fetchBLResponseDTO.panNumber!
                  : "";
    }
  }
  ...
  pushPanDetailsToServer() async {
    PanPostDTO panPostDTO = PanPostDTO();

    panPostDTO.refID = blFetchBloc!.fetchBLResponseDTO.refBlId;
    panPostDTO.dateOfBirth = dateController.text;
    panPostDTO.nameOnPan = firstEditingController.text +
        " " +
        middleEditingController.text +
        " " +
        lastEditingController.text;
    panPostDTO.typeOfUser = "BL";
    panPostDTO.panNumber = aadhaarEditingController.text;

    String data = await panPostDTO.toEncodedJson();
    ...
    businessAadhaarBloc!.addBusinessPanDetails(formData);
  }
